...
- 경로는 측면 거리 0.40/0.70/1.0/1.4m, 추가 깊이 0/0.25/0.55/0.85m의 양방향 후보를 짧은 순으로 검사한다. 인식된 바닥과 몸통 여유를 만족하고 실제 scene-understanding `convexCast`가 통과한 경로만 사용한다. 측정하지 못한 공간의 안전이나 사람의 이동 경로를 보증하지 않는다.
