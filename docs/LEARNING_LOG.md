...
### L-20260920-001 — 게시 대상과 프로젝트 경로

- 상태: 해결
- 재현: 기존 사이트 교체를 별도 소유자 전용 주소로 진행한 뒤, 기존 GitHub Pages가 대상임을 확인했다.
- 관찰: 실제 Pages 설정은 `eunseo-com/2026TechMap_tutorial`, `/2026TechMap_tutorial/`, 공개 workflow 배포다. 로컬 앱 브랜치와 원격 main은 크게 갈라져 있었다.
- 조치: 원격 main 기반 작업 공간에 입문 문서·예제·생성기만 가져오고 본편 앱과 기존 해설은 보존했다. 모든 읽기 페이지 URL에 프로젝트 경로를 적용하고 예전 챕터 URL을 연결했다.
- 검증: 네 페이지의 13개 코드 블록·자산·목차, 다섯 ZIP과 원본 일치 검사 통과. 기존 해설 12개 예제·DocC·이미지 gate도 통과했다.
- 배운 점: 기존 공개 URL과 배포 원본을 확인한 후 호스팅 대상을 결정한다.

### L-20260920-002 — 로컬 포트 제한과 최종 표시 회귀

- 상태: 해결
- 재현: 공개 경로 브라우저 검사 및 기존 심화 문서의 Chapter 1 링크 클릭.
- 관찰: 제한된 실행 환경의 로컬 포트 바인딩은 EPERM을 반환했다. 허용 환경의 검사는 새 읽기 페이지 코드 줄 수와 DocC 주석·링크의 대비 오류 44건을 발견했다. 심화 문서 내부 링크는 SPA 탐색으로 이전 튜토리얼을 표시했다.
- 조치: 로컬 서버가 허용된 환경에서 실행해 검증기 회귀 13/13 통과. 보조 글자 색을 진하게 하고 원문의 주석·링크에는 테마 전경색을 적용했다. 최종 산출물의 심화 문서에서 예전 챕터 링크를 전체 페이지 이동으로 연결한다.
- 검증: 최종 21개 경로×네 렌더 조합 84건, 내부 링크 167개와 no-slash 이동 15개 통과. 문서→새 읽기 페이지 클릭도 네 프로필에서 통과했고, 별도로 네 챕터 실제 클릭을 확인했다. 검증기 회귀 13/13도 통과했다. SDK 검사는 시뮬레이터 실행·실물 AR 확인과 구분한다.
- 배운 점: 정적 링크 존재 검사 외에 실제 문서 내부 이동과 테마별 대비를 확인한다.


### L-20260908-175 — 유한한 방향 벡터의 제곱 길이 overflow

- 상태: 해결
- 재현: 유한하지만 매우 큰 retreat 성분을 가진 plan을 경로 탐색에 전달했다. 길이 제곱이 무한대가 되고 정규화가 퇴화하는 경계를 검사했다.
- 관찰: 55개 실행 중 해당 테스트의 3개 assertion이 실패했다. 이전 구현은 퇴화 경로를 성공으로 반환하고 충돌 callback도 3회 호출했다.
- 조치·검증: 정규화 전에 제곱 길이의 유한성을 검증했다. 최종 `/tmp/piggy-hide-route-policies-final.log`는 55/55·실패 0이며 iPhoneOS Swift 5·6 strict build도 exit 0이다.

### L-20260908-174 — 경로 보수의 문서 검증과 기기 실행 경계

- 상태: 해결 / 실기기 대기
- 재현·관찰: 새 측면 간격 설명으로 DocC content gate의 이전 `0.40/0.70/1.0/1.4m` 조건만 실패했다. 독립 iPhoneOS 예제 타입 검사는 12/12 통과였다. 새 간격·출발점·카메라 거리·실제 첫 점 조건으로 검증 기준을 갱신했다. 추가 문단을 Step 안에 넣은 archive는 `Extraneous element` 경고로 중단됐고, 경로 설명을 Section 본문으로 옮긴 뒤 경고 없이 생성됐다.
- 영향·조치: 문서 형식을 고친 것이며 충돌 검사나 앱 성공 조건은 약화하지 않았다. 파일 검색의 미일치 glob 두 건은 명시 경로로 정정했고, 문서 patch의 불완전한 문맥 매칭 실패는 변경이 적용되지 않았음을 확인한 뒤 실제 문단으로 재적용했다.
- 실기기: paired iPhone을 읽기 전용으로 조회했지만 `passcodeRequired: true`였다. 잠금 우회·기기 설정 변경·Simulator 실행 없이 이번 수정의 physical runtime은 대기한다. 이전 223/223 결과를 새 수정의 실행 결과로 재사용하지 않는다.
- 검증: 문단 정리 후 live obstruction 설명의 검증 문구가 한 차례 누락되어, 이동 중에는 새 충돌 검사를 하고 초기 캐시를 재사용하지 않는다는 경계를 명시했다. `/tmp/piggy-hide-route-docc-content.log` 12/12·exit 0, archive exit 0, 이미지 8개·링크 검사와 44회 브라우저 렌더·내부 링크 101개가 통과했다. 최종 증거 문단 추가 뒤 같은 검증을 다시 수행한다. 최종 앱 Swift 5·6 strict compile/link exit 0; 00:43에도 기기 잠금은 유지되어 최신 230개 전체 runtime·설치는 대기한다.
- 최종 재검증: 증거 문단까지 포함한 content 12/12, warning-free archive, 이미지 8개·링크 gate, **44회 렌더·102개 내부 링크·10개 no-slash 이동**이 모두 exit 0이다. 실행 로그는 `/tmp/piggy-hide-route-docc-{content,build,site,browser}.log`에 보존했다.

### L-20260908-173 — 물체를 탭해도 우회 경로를 만들지 못함

- 상태: 해결 / 실기기 대기
- 재현: 사용자가 실제 앱에서 여러 물체를 인식하고 탭해도 “돌아갈 길이 없다”는 안내로 숨기가 시작되지 않는다고 보고했다.
- 관찰: 후보 모두가 클릭 면에서 0.28m 앞의 동일 출발점에서 시작한다. 아래쪽이 튀어나온 가구는 그 출발점부터 겹칠 수 있다. 측면 후보는 0.40 다음이 0.70m여서 그 사이의 실제 통로를 검색하지 않는다. 바닥 부족으로 후보가 0개인 경우와 실제 충돌 거절이 동일 메시지다.
- 가설: 고정 출발점·드문 측면 표본이 일부 유효한 물체를 반드시 거절한다. 사용자의 실제 frame·물체 종류는 아직 확보하지 않아 유일 원인이라고 단정하지 않는다.
- 조치: `2026-09-08-hide-route-rejection.md`에 따라 실제 production planner와 독립 장애물 계산으로 먼저 RED를 재현한다. 기기 이동을 요청하거나 Simulator로 대체하지 않고, 연결 가능한 physical iPhone은 자동 검증에 사용한다. 조사 중 기본 checkout에서 worktree 파일을 읽으려 한 경로 조회는 실패해 올바른 workdir로 정정했다.
- 검증: `/tmp/piggy-route-red-20260908.log`는 실제 production Swift 50개 중 두 경로의 `XCTUnwrap(nil)` 실패를 재현했다. 기존 48개는 통과했다. 카메라 snapshot·적응형 출발점·0.50/0.60/0.80/1.2m 측면 후보·안내 분리·실제 첫 점 배치 뒤 `/tmp/piggy-hide-route-policies-final.log`는 55/55·실패 0이다. 새 출발점도 카메라와 0.90m 이상, floor footprint와 실제 몸통 검사를 유지한다. 사용자의 실제 가구·mesh에서 재시도는 여전히 실기기 대기다.

### L-20260908-172 — 공개 JSON의 slash 이스케이프를 이전 내용으로 오판함

- 상태: 해결
- 재현: query가 없는 공개 진단 JSON 원문에서 `includes("223/223")`로 검사했다.
- 관찰: HTTP 200이지만 원문은 `223\/223`이므로 단순 문자열 검사가 실패했다. JSON을 파싱한 같은 응답에는 최신 `223/223`이 있다.
- 원인/영향: JSON 직렬화 표현과 실제 문서 내용을 혼동한 검사 문제이며, 배포·앱·DocC 내용의 결함은 아니다.
- 조치·검증: JSON 파싱 후 값으로 검사해 정상 공개 URL의 HTTP 200·최신 223/223을 확인했다. 기존 11경로·8이미지 공개 검사도 처음부터 파싱한 JSON을 사용해 통과했다. 공개 코드나 배포 설정은 변경하지 않는다.
...
- 상태: 해결
- 발생 태스크: AR 사용성 개선 5 — 배포 후 인수인계
- 관찰: 브라우저 연결 초기화 뒤 이전 숫자 식별자로 탭을 선택해 찾지 못했고, 실패한 바인딩을 재사용한 다음 시도도 실패했다. 인수인계 문서 패치는 한 줄의 일부만 context로 지정해 적용 전 검증에서 실패했으며 작업 트리는 변경되지 않았다.
- 영향: 배포·앱·문서 산출물에는 영향이 없고 관찰과 기록 단계만 지연됐다.
- 조치: 현재 브라우저/탭 목록에서 기존 공개 탭을 다시 선택하고 새 핸들을 선언했다. 문서 패치는 실제 전체 행에 맞춰 다시 적용했다. 임시 서버 정리의 프로세스 조회도 기본 격리에서 거절되어, 허용된 조회로 이 작업의 loopback 포트와 임시 디렉터리를 확인한 뒤 해당 프로세스 하나만 종료했다.
- 검증: 공개 소개, 스캔 컨셉·정확한 챕터 라벨·연결 코드를 실제 브라우저에서 확인했고 공개 경로 11개와 이미지 8/8 해시도 통과했다. 기록 변경은 diff 검사 뒤 별도 문서 커밋으로 보존한다.
