...
## 6. Blender Z-up 에셋을 그대로 놓으면 바닥은 서고 돼지는 바닥 아래로 파묻힘

**증상**: `Ground_Color.usdc`를 방의 `Floor`로 바로 추가하면 높이 2m짜리 세로 판처럼 렌더링되어 카메라 시야를 가렸다. `Piggy.usdc`는 높이를 정규화했는데도 바닥면이 y=0보다 아래에 남아, 실제 화면에서 돼지가 프레임 하단에 거의 보이지 않았다.

**추적**: USD 메타데이터에서 `Ground_Color`의 `upAxis`가 Z임을 확인했다. C3_Piggy의 `IslandBuilder.flatGroundMetrics`는 바닥을 X축 -90°로 눕힌 **뒤** 실제 경계를 재며, `PigController.loadPigModel`은 돼지에 X축 +90°·Z축 180° 회전을 적용한 뒤 바닥을 정렬한다. 둘 다 하위 메시의 꼭짓점을 컨테이너 좌표계로 변환해 경계를 구한다.

**원인**: SceneKit은 에셋 파일의 제작 좌표계를 자동으로 월드의 Y-up 방향으로 바로잡아 주지 않는다. 또한 `SceneKitGeometry.boundingBox(of:)`는 전달받은 노드의 **자식** 변환을 반영해 측정하므로, 같은 노드에 회전·스케일을 적용한 뒤 다시 자기 자신을 측정하면 그 루트 변환은 측정에 포함되지 않는다.

**해결**: 바닥은 이름 있는 컨테이너 안에 실제 모델을 넣고, 모델을 X축 -90° 회전한 다음 컨테이너 기준 경계를 측정했다. 컨테이너에 x/z는 방의 폭·깊이, y는 0.1m 두께가 되도록 스케일을 적용하고 하단이 y=0이 되도록 위치를 조정했다. 돼지는 바깥 노드(위치·HideAction)와 안쪽 모델 노드(회전·스케일·바닥 정렬)를 나눴다. 이렇게 하면 이동 액션이 모델 보정값을 덮어쓰지 않는다. 초기 카메라 프레임에는 돼지가 너무 가까웠으므로, 의미가 임의인 하드코딩 z 좌표만 1에서 0으로 조정했다.

**주의**: "높이를 정규화했다"는 사실만으로 세워진 모델이나 바닥 접촉을 보장할 수 없다. 회전이 필요한 에셋은 **회전 → 경계 측정 → 스케일 → 바닥/중심 정렬** 순서를 지키고, 실제 씬 좌표계에서 폭·높이·깊이·하단 위치를 모두 검사하는 회귀 테스트를 둔다.

## 7. 새 XCTest 파일을 만들었는데 선택 테스트가 0개 실행됨

**증상**: `ClosedWorldSceneViewTests.swift`를 추가한 직후 `-only-testing:...` 명령이 성공했지만, 결과는 `Executed 0 tests`였다.

**원인**: Tuist가 생성해 둔 `PiggyEscape.xcodeproj`에는 새 파일이 아직 등록되지 않았다. 컴파일 오류가 아니어서 명령 종료 코드만 보면 테스트가 통과한 것처럼 보였다.

**해결**: `tuist generate --no-open`으로 생성 프로젝트를 갱신한 뒤 같은 선택 테스트를 다시 실행했다. 이때 실제로 1개 테스트가 실행·통과하는 것을 확인했다.

**주의**: 새 소스나 테스트 파일을 추가한 뒤에는 `xcodebuild`의 성공 여부뿐 아니라 **실행된 테스트 수**를 확인한다. 특히 `-only-testing`은 이름이 틀리거나 생성 프로젝트가 오래됐어도 0개 실행으로 끝날 수 있다.

## 8. Simulator에서 실제 물체 뒤 오클루전을 확인할 수 없음

**증상**: 전체 XCTest와 Simulator build가 성공해도, 실제 물체를 LiDAR로 재구성해 돼지를 가리는 장면은 관찰할 수 없다.

**원인**: `ARWorldTrackingConfiguration`의 실제 분류 메쉬와 물리 공간은 Simulator 입력이 아니다. 테스트는 수직 면·바닥·거리 계산과 blocked→visible 한 번 재발견 규칙만 결정론적으로 확인한다.

**해결/주의**: Simulator 성공을 실제 오클루전 통과로 기록하지 않는다. LiDAR 지원 기기에서 실제 물체의 옆면을 탭하고, 초기 위치에서는 가려지며 사용자가 옆으로 이동한 뒤에만 다시 보이는지를 별도 확인한다. 관찰 전 상태는 `실기기 대기`다.

## 9. 시스템 카메라 권한 화면은 실기기에서만 완전히 확인할 수 있음

**증상**: 주입한 권한 결과로 허용·거부·제한 상태 전이는 테스트할 수 있지만, 시스템 목적 문구와 실제 앱 복귀 흐름은 Simulator 또는 단위 테스트만으로 확정할 수 없다.

**원인**: 시스템 권한 알림은 기기별 권한 상태와 운영체제 UI가 소유한다.

**해결/주의**: C3 발견 뒤 0.70초 페이드가 끝난 후 한 번만 요청하도록 코드로 제한하고, 실제 기기에서 허용·거부·제한·Settings 복구를 관찰한다. 관찰 전에는 `실기기 대기`로 남긴다.

## 10. ARView 화면 확대는 렌즈나 FOV를 바꾸지 않음

**증상**: 현실에서 다시 들킨 순간 화면이 1.12배로 커지지만, 물체 뒤 오클루전의 카메라 기하를 바꾸면 안 된다.

**원인**: AR 카메라 transform·렌즈를 바꾸면 실제 메쉬와 돼지의 공간 관계가 달라져 “사용자가 몸을 움직여 찾아낸다”는 경험 계약이 무너질 수 있다.

**해결/주의**: `ARView` 자체가 아닌 SwiftUI 컨테이너에만 1.12→1.0 `scaleEffect`를 적용한다. 실제 화면에서 체감 품질과 1.5배 돼지 확대의 동시성은 LiDAR 실기기에서 별도 확인한다.
